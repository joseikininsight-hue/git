...
function gi_optimize_lcp() {
    ...
    if (is_singular('grant')) {
        echo '<link rel="preload" href="https://fonts.gstatic.com/s/notosansjp/v52/-F6jfjtqLzI2JPCgQBnw7HFyzSD-AsregP8VFBEj75s.woff2" as="font" type="font/woff2" crossorigin>' . "\n";
    }
}

/**
 * CLS (Cumulative Layout Shift) Prevention
 * Add aspect ratio and dimensions to prevent layout shifts
 */
add_action('wp_head', 'gi_prevent_cls', 3);
function gi_prevent_cls() {
    ?>
    <style id="cls-prevention-css">
...
/**
 * Internal Link Structure Enhancement (Phase 2)
 * Add contextual internal links to improve SEO
 */
add_action('wp_footer', 'gi_internal_link_enhancement', 5);
function gi_internal_link_enhancement() {
    // Only on grant pages
    if (!is_singular('grant')) {
        return;
    }
    ...
    // Get related grants for internal linking
    $categories = wp_get_post_terms($post_id, 'grant_category', array('fields' => 'ids'));
    ...
    $related_grants = new WP_Query(array(
        'post_type' => 'grant',
        'posts_per_page' => 5,
        'post__not_in' => array($post_id),
        'post_status' => 'publish',
    ...
    // Output structured internal links data for SEO
    ?>
    <script type="application/ld+json">
    {
        ...
        "itemListElement": [
            ...
            $items = array();
            $position = 1;
            while ($related_grants->have_posts()) {
                $related_grants->the_post();
                $items[] = sprintf(
                    '{"@type": "ListItem", "position": %d, "item": {"@type": "GovernmentService", "name": %s, "url": "%s"}}',
                    $position,
                    json_encode(get_the_title(), JSON_UNESCAPED_UNICODE),
 ...
 */
add_action('wp_head', 'gi_add_resource_hints', 1);
function gi_add_resource_hints() {
 ...
 */
add_filter('style_loader_tag', 'gi_optimize_style_loading', 10, 4);
function gi_optimize_style_loading($tag, $handle, $href, $media) {
    ...
    if (in_array($handle, $async_styles) && !is_admin()) {
        // Use media="print" onload trick for async CSS
        $tag = str_replace(
            "media='all'",
            "media='print' onload=\"this.media='all'\"",
            $tag
 ...
 */
add_filter('wp_get_attachment_image_attributes', 'gi_smart_lazy_loading', 20, 3);
function gi_smart_lazy_loading($attr, $attachment, $size) {
    // Check if this is a hero/above-the-fold image
    $above_fold_classes = array(
        'hero__image',
        'ji-logo-image',
        'featured-image',
        'grant-hero',
