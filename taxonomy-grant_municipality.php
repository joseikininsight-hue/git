<?php
/**
 * Municipality Archive Template for Grant - 図鑑風デザイン統一版
 * 市町村別助成金アーカイブページ - archive-grant.phpベース
 * 
...
$municipality_name = $current_term->name;
$municipality_slug = $current_term->slug;
$municipality_description = $current_term->description;
$municipality_count = $current_term->count;
$municipality_id = $current_term->term_id;
...
// タイトル・説明文の生成
$archive_title = $municipality_name . 'の補助金・助成金';
$archive_description = $municipality_description ?: 
    $municipality_name . 'で利用できる助成金・補助金の最新情報。' . 
    number_format($municipality_count) . '件の支援制度を掲載。' .
    ($parent_prefecture ? $parent_prefecture['name'] . 'の制度と合わせて活用可能。' : '') .
    '専門家による申請サポート完備。';

// 総件数
$total_grants = $municipality_count;
...
if ($parent_prefecture) {
    $breadcrumbs[] = array('name' => $parent_prefecture['name'], 'url' => get_term_link($parent_prefecture['slug'], 'grant_prefecture'));
}

$breadcrumbs[] = array('name' => $municipality_name, 'url' => '');
...
<?php
// title タグ用
$seo_title = $municipality_name . 'の補助金・助成金一覧【' . $current_year . '年度最新版】' . number_format($municipality_count) . '件';
...
$schema_data = array(
    ...
    'mainEntity' => array(
        ...
        'name' => $municipality_name . 'の助成金・補助金一覧'
    )
);

if ($parent_prefecture) {
    $schema_data['spatialCoverage'] = array(
        '@type' => 'City',
    ...
    <section class="zukan-hero zukan-hero-simple">
        <div class="yahoo-container">
            <div class="zukan-hero-content">
                <span class="zukan-hero-label">Municipality Archive</span>
            ...
            <section class="gi-section" id="list">
                <header class="gi-section-numbered-header">
                    ...
                    <div class="gi-section-title-box">
                        ...
                        <h2 class="gi-section-title"><?php echo esc_html($municipality_name); ?>の補助金図鑑一覧</h2>
            ...
            <section class="zukan-article-section" id="guide">
                ...
                <article class="zukan-article-content">
                    <div class="zukan-article-header">
                        ...
                        <?php echo esc_html($municipality_name); ?>の助成金・補助金申請ガイド
                    </div>
                    
                    <div class="zukan-article-columns">
                        <div>
                            <h3>壱. 申請の傾向</h3>
                            <p><?php echo esc_html($municipality_name); ?>では、地域の産業振興や中小企業支援を目的とした独自の助成金制度が充実しています。特に創業支援、事業承継、設備投資に関する支援が手厚く、申請件数も年々増加傾向にあります。審査では地域経済への貢献度や雇用創出効果が重視される傾向があります。</p>
                        </div>
                        <div>
                            <h3>弐. 採択のポイント</h3>
                            <p>採択率を高めるためには、事業計画の具体性と実現可能性が鍵となります。また、<?php echo esc_html($municipality_name); ?>の産業政策との整合性を示すことも重要です。申請書類では、数値目標を明確に設定し、その達成に向けた具体的なアクションプランを提示することをお勧めします。</p>
                        </div>
                    </div>
