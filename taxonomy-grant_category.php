<?php
/**
 * Category Archive Template for Grant - 図鑑風デザイン統一版
 * カテゴリー別助成金アーカイブページ - archive-grant.phpベース
 * 
...
// タイトル・説明文の生成
$archive_title = $category_name . 'の補助金・助成金';
$archive_description = $category_description ?: 
    $category_name . 'に関する助成金・補助金の情報を' . number_format($category_count) . '件掲載。' . 
    '申請方法から採択のコツまで、専門家監修の最新情報を提供しています。毎日更新。';

// 総件数
$total_grants = $category_count;
...
$breadcrumbs = array(
    ...
    array('name' => 'カテゴリ一覧', 'url' => ''),
    array('name' => $category_name, 'url' => '')
);

// 親カテゴリーがあれば追加
if ($current_term->parent > 0) {
    $parent_term = get_term($current_term->parent, 'grant_category');
    if ($parent_term && !is_wp_error($parent_term)) {
        // パンくずに親カテゴリを挿入
        array_splice($breadcrumbs, 2, 1, array(
            array('name' => 'カテゴリ一覧', 'url' => ''),
            array('name' => $parent_term->name, 'url' => get_term_link($parent_term)),
        ));
    }
}
...
<?php
// title タグ用
$seo_title = $category_name . 'の補助金・助成金一覧【' . $current_year . '年度最新版】' . number_format($category_count) . '件';
    ...
    <section class="zukan-hero zukan-hero-simple">
        <div class="yahoo-container">
            <div class="zukan-hero-content">
                <span class="zukan-hero-label">Category Archive</span>
            ...
            <section class="zukan-article-section" id="guide">
                ...
                <article class="zukan-article-content">
                    <div class="zukan-article-header">
                        ...
                        <?php echo esc_html($category_name); ?>の申請傾向と採択のポイント
                    </div>
                    
                    <div class="zukan-article-columns">
                        <div>
                            <h3>壱. この分野の特徴</h3>
                            <p><?php echo esc_html($category_name); ?>関連の助成金は、技術革新や事業効率化を促進することを目的としています。近年は特にデジタル化やサステナビリティへの取り組みに対する支援が拡充されており、申請の機会が広がっています。補助率も比較的高く設定されているケースが多いのが特徴です。</p>
                        </div>
                        <div>
                            <h3>弐. 申請時の注意点</h3>
